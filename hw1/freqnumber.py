    ...
    def low_level_sort(lst, idx, length):
        def compare(item, lst, acc, idx, length) -> List[Tuple[str, int]]:
            if idx == length:
                return acc
            if lst[idx][1] >= item[1]:
                return compare(item, lst, acc + [item], idx + 1, length)
            return compare(item, lst, acc + [lst[idx]], idx + 1, length)
        return reduce(lambda x, y: compare(y, lst, [], 0, len(lst)), lst, [])
...
def main():
    ...
    def allowed(string):
        return len(list(filter(lambda x: 48 <= ord(x) <= 57 or ord(x) == 46 or ord(x) == ord('-'), string))) == len(string)
    numbers = list(
                filter(
                    allowed,
                    reduce(
    ...
    unique_numbers = list(
            filter(
                ...
                reduce(
                    lambda xt, yt: xt + reduce(
                        lambda x, y: x + [y] if y not in x and y not in xt else x,
                        yt.split(' '),
                        []
                    ),
                    lines,
    ...
    with open(output_file, 'w') as OFS:
        def output_to_file_int(idx, k_val, contents):
            if k_val == 0 or idx == -1:
                OFS.write(contents)
                return
            output_to_file_int(idx - 1, k_val - 1, contents + f"{integers_sorted[idx][0]} {integers_sorted[idx][1]}\n")
        def output_to_file_real(idx, k_val, contents):
            if k_val == 0 or idx == -1:
                OFS.write(contents)
                return
            contents + f"{reals_sorted[idx][0]} {reals_sorted[idx][1]}\n"
            output_to_file_real(idx - 1, k_val - 1, contents + f"{reals_sorted[idx][0]} {reals_sorted[idx][1]}\n")
